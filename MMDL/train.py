...
def main():
    ...
    if args.evaluate:
        ...
        for dataloader in [test_loader]:
            ...
            outputs = validate(dataloader, net, criterion, epoch, logger, args)
            score_icetype.append(F1_score_icetype)
            micro_score_icetype.append(F1_score_micro_average_icetype)
            macro_score_icetype.append(F1_score_macro_average_icetype)

            score_icecon.append(F1_score_icecon)
            micro_score_icecon.append(F1_score_micro_average_icecon)
            macro_score_icecon.append(F1_score_macro_average_icecon)
        # print("F1 score:", score)
        # print("micro score:", micro_score)
        # print("macro score:", macro_score)
        save_filename = './performance/%s.hdf' % args.save_name
        with h5py.File(save_filename, 'w') as f:
            f.create_dataset("F1_score_icetype", data=score_icetype)
            f.create_dataset("micro_score_icetype", data=micro_score_icetype)
            f.create_dataset("macro_score_icetype", data=macro_score_icetype)

            f.create_dataset("F1_score_icecon", data=score_icecon)
            f.create_dataset("micro_score_icecon", data=micro_score_icecon)
            f.create_dataset("macro_score_icecon", data=macro_score_icecon)
...
def train(train_loader, net, criterion, optimizer, epoch, logger, args):
    ...
    for i, (images, target, target_con, location) in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast():
            ...
            loss = (criterion(output_icetype, target) + criterion(output_icecon, target_con)) * 0.5
