...
class ResNet(nn.Module):
    def __init__(self,
                 ...
                 layers: List[int],
                 num_classes_icetype: int = 11,
                 num_classes_icecon: int = 12,
                 num_channels: int = 2,
        ...
        self.fc_icetype = nn.Linear(512 * block.expansion, num_classes_icetype)
        self.fc_icecon = nn.Linear(512 * block.expansion, num_classes_icecon)
    ...
    def forward(self, x):
        ...
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
        icetype = self.fc_icetype(x)
        icecon = self.fc_icecon(x)
...
def resnet18(logger, args):
    ...
    model = _resnet('resnet18', BasicBlock, [2, 2, 2, 2],
...
if __name__ == "__main__":
    ...
    model = resnet18(pretrained=False, num_classes=11, num_channels=2)
    summary(model, (2, 30, 30))